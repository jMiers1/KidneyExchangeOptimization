...
struct state{
    ...
    double MaxWeight;
    std::vector<int>::iterator Itestoy;
    std::vector<int>::iterator ItEnd;
    std::vector<int>::iterator ItmActualCualestoy; //
    std::vector<int>::iterator ItmActualCualEnd;
    std::map<int,bool> vVisited;   // visited arcs
    std::vector<int> vSuce;
    std::vector<int> vPrede;
    std::map<int,bool>NoMergeables;
    bool firstTime;
    bool pathLlego;
    int predLongest; //position of state giving the longest path in a previous layer
    int mActualCual;
    std::vector<int> VpredLongest;
    std::vector<int> VmActualCual;
    bool cadeRepe;
    bool meConectoDos;
    int totalVeci;
    int vamosVeci;
    std::vector<std::tuple<int,int,double> > vTupleVarios;
    //////Functions//////
    state (int _id, int _actu, std::map<int,std::vector<int> >& Comp, state& vengo);
    state (int _id, int _origen, std::map<int,std::vector<int> >& Comp);
    void limpiar();
    void sizeBytes();
    void eliminar88(std::vector<state>& vLosNew);
...
struct vChain{
    int vertex;
    bool FirstPass = true;
    std::vector<int>e_sol;
    std::vector<int>::iterator it = e_sol.begin();
    std::vector<int>::iterator itEnd = e_sol.end();
    vChain(int _vertex, std::vector<int>sol){vertex = _vertex, e_sol = sol;}
};
...
public:
    ...
    std::vector<std::vector<pair<int,double> > > LongestPath;
    std::vector<std::map<int,vector<int> > >JustVisitedByState; // level<actual,vector<pos> >
    std::vector<std::pair<vector<state>, vector<vector<int> > > > vMDD;   //vector with copies, first vector<state> and second vector<vector<int> > vLvEstados
    std::vector<std::pair<int,int> > ordenFor;
    std::tuple<int,int,double,int,double> tCicloFin;
    std::vector<std::tuple<int,int,double,int,double> > ttCicloFin;
    void CreateIncumbent(IloNumVarArray& z, vector<int> thisSol);
    std::vector<std::vector<int> >LeftCycles();
    ...
    int DijktrasAlgorithm(map<int,vector<int> >& Comp2, int source, int sink);  // Julius: commented out on 02.11
    void BuildMDDs();
    void llenarComp();
    void Lagrange();
    ...
    double Heuristic(); // Julius: commented out on 02.11
