...
pair<vector<vector<pair<int, int>>>, vector<vector<int>>> findViolatingCycles(const vector<pair<int, int>>& edgesInSolution,
                                                                              const vector<vector<int>>& adjacencyList, 
                                                                              const int& k) {
    vector<vector<pair<int, int>>> violatingEdgeCycles;
    vector<vector<int>> violatingNodeCycles;

    set<int> exploredNodes; // Store already explored nodes

    for (int startNode = 0; startNode < adjacencyList.size(); ++startNode) {
        if (exploredNodes.count(startNode)) {
            continue; // Skip nodes that are already explored
        }

        vector<int> currentNodeExploration; // Path/cycle generated by the current node
        vector<pair<int, int>> currentEdgeExploration;
        exploredNodes.insert(startNode);
        currentNodeExploration.push_back(startNode);

        int currentNode = startNode;
        auto successors = adjacencyList[currentNode];

        while (!successors.empty() && !exploredNodes.count(successors[0])) {
            // Node has an unexplored successor
            int succ = successors[0];
            currentNodeExploration.push_back(succ);
            currentEdgeExploration.push_back({currentNode, succ});
            exploredNodes.insert(succ);
            currentNode = succ;
            successors = adjacencyList[currentNode];
        }

        // Check for cycles
        if (!successors.empty() && 
            find(currentNodeExploration.begin(), currentNodeExploration.end(), successors[0]) != currentNodeExploration.end()) {
            // A cycle is found
            int succ = successors[0];
            currentEdgeExploration.push_back({currentNode, succ});
            if (currentNodeExploration.size() > k) {
                violatingEdgeCycles.push_back(currentEdgeExploration);
                violatingNodeCycles.push_back(currentNodeExploration);
            }
        }
    }

    return {violatingEdgeCycles, violatingNodeCycles};
}
